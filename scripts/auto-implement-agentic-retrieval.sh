...
TASKS_FILE="./agentic-retrieval-tasks.json"
PLAN_FILE="./implementation-agentic-retrieval.md"
DEV_LOG="./agentic-retrieval-dev-log.txt"
...
select_tasks_with_sonnet() {
  ...
  local prompt
  prompt=$(cat <<EOF
You are reviewing the backlog for pi-orchestra agentic retrieval work.

Read these files for context:
- ./agentic-retrieval-dev-log.txt
- ./implementation-agentic-retrieval.md
- ./agentic-retrieval-tasks.json
...
- use jq to directly update ./agentic-retrieval-tasks.json so the selected tasks are marked with status "active"
- after updating the file, respond with ONLY the selected task IDs as valid JSON
- respond with ONLY a JSON array of task IDs, for example: ["AR-P1-T1","AR-P1-T2"]
...
implement_active_tasks_with_opus() {
  ...
  local prompt
  prompt=$(cat <<EOF
You are implementing the currently active tasks for pi-orchestra agentic retrieval work.
...
- keep work aligned with ./implementation-agentic-retrieval.md
